...
class Debug:      
    def __init__( self,
        ...
        if time_stamp not in (0,1,2):
            msg2 = '%s' % time_stamp
            raise ValueError('Invalid time_stamp param: %s' % msg2)
        self.prefix = prefix
        self.sufix = sufix
        ...
        if type(flag_show) in (type(''), type(None)):
            ...
            raise ValueError('Invalid type for flag_show!: %s' % msg2)
    ...
    def _append_unique_str( self, lst, item ):
        ...
        if type(item) != type(''):
            msg2 = '%s' % item
            raise TypeError('Invalid item type (should be string): %s' % msg2)
        if item not in lst:
            lst.append( item )
        return lst
        ...
        if flags:
            for f in self._as_one_list( flags ):
                if not f in self.debug_flags:
                    msg2 = '%s' % f
                    raise ValueError('Invalid debugflag given: %s' % msg2)
