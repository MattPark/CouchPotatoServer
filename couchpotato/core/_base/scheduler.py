...
class Scheduler(Plugin):
    ...
    def remove(self, identifier):
        for cron_type in ['intervals', 'crons']:
            try:
                self.sched.remove_job(identifier)
    ...
    def cron(self, identifier = '', handle = None, day = '*', hour = '*', minute = '*'):
        ...
        self.sched.add_job(handle, 'cron', day=day, hour=hour, minute=minute, id=identifier, replace_existing=True)
    ...
    def interval(self, identifier = '', handle = None, hours = 0, minutes = 0, seconds = 0):
        ...
        self.remove(identifier)
        self.sched.add_job(handle, 'interval', hours=hours, minutes=minutes, seconds=seconds, id=identifier, replace_existing=True)
