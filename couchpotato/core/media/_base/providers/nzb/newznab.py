...
class Base(NZBProvider, RSS):
        ...
        for nzb in nzbs:
            ...
            if "@spot.net" in nzb_id:
                try:
                    ...
                    password = re.search(r'(?:' + self.passwords_regex + r')(?: *)(?:\:|\=)(?: *)(.*?)\<br\>|\n|$', description, flags = re.I).group(1)
                    if password:
    ...
    def getHosts(self):
        ...
        for nr in range(len(hosts)):
            ...
            except IndexError: score = 0

            try: custom_tag = custom_tags[nr]
            except IndexError: custom_tag = ''

            try: custom_category = custom_categories[nr].replace(" ", ",")
            except IndexError: custom_category = ''
    ...
    'groups': [
        {
            ...
            'options': [
                ...
                },
                {
                    'name': 'custom_tag',
                    'advanced': True,
                    'label': 'Custom URL Params',
                    'default': ',,,,,',
                    'description': 'Extra URL parameters appended to API requests. Examples: rls=1 (scene releases only on nzbs.org), attrs=poster (include poster URL)',
                },
                {
                    ...
                    'description': 'Newznab category IDs (space-separated for multiple). Default 2000 = all movies. Common: 2030=SD, 2040=HD, 2045=UHD, 2050=BluRay, 2060=3D. Example: "2040 2045 2050" for HD + UHD + BluRay only',
                },
                {
                    'name': 'api_key',
                    'default': ',,,,,',
                    ...
                    'type': 'combined',
                    'combine': ['use', 'host', 'api_key', 'extra_score', 'custom_tag', 'custom_categories'],
                },
            ],
        },
    ],
