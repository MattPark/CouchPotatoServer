...
var CouchPotato = new Class({
    ...
    createUserscriptButtons: function() {
        ...
        return new Element("div.group_userscript").adopt(new Element("div.bookmarklet_wrapper").adopt(new Element("span.bookmarklet").adopt(new Element("a.button", {
            text: "+CouchPotato",
...
Page.Settings = new Class({
        ...
        options.stableSort(self.sortByOrder).each(function(section) {
            ...
            section.groups.stableSort(self.sortByOrder).each(function(group) {
                ...
                if (group.type && group.type == "list") {
                    new Element("fieldset.list_section_header").adopt(new Element("h2").adopt(new Element("span.group_label", { text: group.label || (group.name || "").capitalize() }), group.description ? new Element("span.hint", { html: group.description }) : null)).inject(content_container);
                    if (!self.lists[group.name]) { self.lists[group.name] = self.createList(content_container, group.name); } else { self.lists[group.name].inject(content_container); }
                    return;
                }
                ...
                if (!self.tabs[group.tab].groups[group.name]) {
                    var group_el = self.createGroup(group).inject(group.list ? self.lists[group.list] : content_container).addClass("section_" + section_name);
                    if (group.multi_instance === false) group_el.set("data-no-duplicate", "1");
                    self.tabs[group.tab].groups[group.name] = group_el;
                    if (group.list && self.getValue(section_name, "hidden") == "1") { group_el.addClass("provider-removed"); }
                }
    ...
    createList: function(content_container, list_name) {
        ...
        var dropdown = new Element("select", {"events": {"change": function() {
            ...
            if (action === "enable") {
                ...
                fieldsets.each(function(fs) {
                    ...
                    if (label && label.get("text").trim() === target) {
                        ...
                        if (toggle) { toggle.set("checked", true); toggle.fireEvent("change"); }
                        fs.removeClass("provider-removed");
                        fs.removeClass("disabled");
                        var fs_section = ""; fs.get("class").split(" ").each(function(cls) { if (cls.indexOf("section_") === 0) fs_section = cls.replace("section_", ""); });
                        if (fs_section) { Api.request("settings.save", {"data": {"section": fs_section, "name": "hidden", "value": "0"}}); }
                    }
                });
        ...
        list_el.addEvent("refreshDropdown", function() {
            ...
            all_fieldsets.each(function(fs) {
                ...
                fs.get("class").split(" ").each(function(cls) {
                    if (cls.indexOf("section_") === 0) { section_class = cls.replace("section_", ""); }
                });
                ...
                if (has_visible) {
                    if (is_notification) {
                        ...
                        if (!no_dup) {
                            dropdown.adopt(new Element("option", {"value": "duplicate:" + base_type, "text": display_name}));
                        }
                    }
                } else {
                    dropdown.adopt(new Element("option", {"value": "enable:" + display_name, "text": display_name}));
                }
            });
                ...
                "events": {
                    "click": function(e){
                        ...
                        if(is_notification && /\_\d+$/.test(self.section)){
                            ...
                            Api.request("settings.save", {"data": {"section": self.section, "name": "hidden", "value": "1"}});
                        }
                    }
                }
            }).inject(self.el, "top");
...
Option.Choice = new Class({
    ...
    afterInject: function() {
        ...
        if (o.presets) {
            var preset_wrapper = new Element("div.select_wrapper.icon-dropdown.preset_wrapper").grab(self.preset_select = new Element("select.select", {
                events: {
                    change: self.applyPreset.bind(self)
                }
            }).grab(new Element("option[text=Presets]")));
...
Option.Apprise_urls = new Class({
    ...
    create: function() {
        ...
        self.input = new Element("input", {type: "hidden", name: self.postName(), value: self.getSettingValue() || "[]"});
        self.rows_el = new Element("div.apprise-rows");
        self.add_btn = new Element("a.button.apprise-add-btn", {text: "+ Add Service", events: {click: function(e) {
            e.preventDefault();
            self._createRow("", "", true, false);
            self._saveAll();
        }}});
        self.el.adopt(self.input, self.rows_el, new Element("div.apprise-footer").adopt(self.add_btn));
    },
    afterInject: function() {
        var self = this;
        self._rows = [];
        document.addEvent("click", function(e) {
            self.rows_el.getElements(".apprise-dropdown-list").each(function(dd) {
                ...
                if (wrapper && !wrapper.contains(e.target)) dd.setStyle("display", "none");
            });
        });
        Api.request("apprise.schemas", {onComplete: function(json) {
            self._schemas = (json.success ? json.schemas : []);
            var entries = [];
            try { var val = self.getSettingValue(); if (val) entries = JSON.parse(val); } catch(e) {}
            if (entries && entries.length) {
                entries.each(function(entry) {
    ...
    _createRow: function(schema, url, enabled, on_snatch) {
        ...
        var service_input = new Element("input.apprise-service-input", {type: "text", placeholder: "Search services...", value: self._getServiceName(schema)});
        ...
        service_input.addEvents({
            focus: function() { dropdown_list.setStyle("display", "block"); self._filterDropdown(dropdown_list, this.get("value")); },
            keyup: function(e) { if (e.key === "escape") { dropdown_list.setStyle("display", "none"); return; } self._filterDropdown(dropdown_list, this.get("value")); }
        });
        service_wrapper.adopt(service_input, dropdown_list);
        var builder_wrapper = new Element("div.apprise-builder-wrapper");
        var builder_link = new Element("a.button.apprise-builder-link", {href: self._getBuilderUrl(schema), target: "_blank", title: "Open URL Builder", text: "Wizard"});
        builder_wrapper.adopt(builder_link);
        var url_input = new Element("input.apprise-url-input", {type: "text", value: url, placeholder: self._getTemplate(schema)});
        url_input.addEvents({change: function() { self._saveAll(); }, keyup: function() { self._saveAll(); }});
        var snatch_wrapper = new Element("div.apprise-snatch-wrapper.advanced", {title: "Also notify when a movie is snatched"});
        var snatch_check = new Element("input", {type: "checkbox", checked: !!on_snatch, events: {change: function() { self._saveAll(); }}});
        ...
        var test_btn = new Element("a.button.apprise-test-btn", {text: "Test", events: {click: function(e) {
            e.preventDefault();
            self._testUrl(url_input.get("value"), this, test_result);
        }}});
        test_wrapper.adopt(test_btn, test_result);
        var delete_btn = new Element("a.icon-delete.apprise-delete", {title: "Remove", events: {click: function(e) {
            e.preventDefault();
            row.destroy();
            self._rows.erase(row);
            self._saveAll();
        ...
        var toggle_input = new Element("input", {type: "checkbox", checked: enabled, events: {change: function() {
    ...
    _getServiceName: function(schema) {
        ...
        var found = "";
        this._schemas.each(function(s) { if (s.schemas.indexOf(schema) !== -1) found = s.service_name; });
        return found;
    },
    _getTemplate: function(schema) {
        if (!schema || !this._schemas) return "schema://...";
        var found = "";
        this._schemas.each(function(s) { if (s.schemas.indexOf(schema) !== -1) found = s.template; });
        return found || schema + "://...";
    },
    ...
    _testUrl: function(url, btn, result_el) {
        if (!url) { result_el.set("html", '<span class="apprise-fail" title="No URL entered">\u2716</span>'); return; }
        btn.set("text", "...");
        result_el.set("html", "");
        Api.request("apprise.test_url", {data: {url: url}, onComplete: function(json) {
            btn.set("text", "Test");
            if (json.success) {
    ...
    _saveAll: function() {
        ...
        self._rows.each(function(row) {
            ...
            entries.push({schema: schema, url: url_input ? url_input.get("value").trim() : "", enabled: toggle_input ? !!toggle_input.checked : true, on_snatch: snatch_input ? !!snatch_input.checked : false});
        });
        self.input.set("value", JSON.stringify(entries));
        self.changed();
    },
...
var AboutSettingTab = new Class({
    ...
    createAbout: function() {
        ...
        self.settings.createGroup({
            ...
            text: "Loading..."
        }), (!self.hide_about_update ? [new Element("dt"), self.update_dd = new Element("dd").adopt(self.update_link = new Element("a.check_for_updates", {
            text: "Check for updates",
            events: {
        ...
        }))] : null), new Element("dt[text=Branch]"), self.branch_text = new Element("dd.branch", {
            text: "Loading..."
        }), new Element("dt[text=Updater]"), self.updater_type = new Element("dd.updater", {
            text: "Loading..."
        }), new Element("dt[text=PID]"), new Element("dd", {
...
var MetadataSettingTab = new Class({
    ...
    startAutoRefresh: function() {
        ...
        self.refresh_timer = setInterval(self.refresh.bind(self), 3e4);
    },
    renderStats: function(json) {
        ...
        if (json.omdb && self.stats_panels["omdbapi"]) {
            ...
            if (o.rate_limited) { bar_class = "bar_depleted"; bar_text = "Rate limited by OMDB (" + o.calls_today + " / " + hard_cap + ")"; }
            else if (o.calls_today >= o.budget) { bar_class = "bar_warning"; bar_text = "Soft budget reached (" + o.calls_today + " / " + o.budget + "), hard cap " + hard_cap; }
            else if (o.calls_today > o.budget * 0.8) { bar_class = "bar_caution"; bar_text = o.calls_today + " / " + o.budget + " budget (" + hard_cap + " hard cap)"; }
            else { bar_class = "bar_ok"; bar_text = o.calls_today + " / " + o.budget + " budget (" + hard_cap + " hard cap)"; }
            self.stats_panels["omdbapi"].set("html", '<div class="stats_grid">' + '<div class="stat_item"><span class="stat_value">' + o.calls_today + '</span><span class="stat_label">Calls Today</span></div>' + '<div class="stat_item"><span class="stat_value">' + o.budget_remaining + '</span><span class="stat_label">Remaining</span></div>' + '<div class="stat_item"><span class="stat_value">' + o.cache_hits_today + '</span><span class="stat_label">Cache Hits</span></div>' + '<div class="stat_item"><span class="stat_value">' + tier_label + '</span><span class="stat_label">Tier</span></div>' + '</div>' + '<div class="stats_bar_wrap"><div class="stats_bar_soft_mark" style="left:' + soft_pct + '%"></div><div class="stats_bar ' + bar_class + '" style="width:' + fill_pct + '%"></div>' + '<span class="stats_bar_text">' + bar_text + "</span></div>");
        }
        if (json.tmdb && self.stats_panels["themoviedb"]) {
            var t = json.tmdb;
            self.stats_panels["themoviedb"].set("html", '<div class="stats_grid">' + '<div class="stat_item"><span class="stat_value">' + t.calls_today + '</span><span class="stat_label">Calls Today</span></div>' + '<div class="stat_item"><span class="stat_value">' + (t.has_custom_key ? "Custom" : "Built-in") + '</span><span class="stat_label">Key Type</span></div>' + "</div>");
        }
        if (json.fanarttv && self.stats_panels["fanarttv"]) {
            var f = json.fanarttv;
            self.stats_panels["fanarttv"].set("html", '<div class="stats_grid">' + '<div class="stat_item"><span class="stat_value">' + f.calls_today + '</span><span class="stat_label">Calls Today</span></div>' + '<div class="stat_item"><span class="stat_value">' + (f.has_custom_key ? "Custom" : "Built-in") + '</span><span class="stat_label">Key Type</span></div>' + "</div>");
        }
        if (json.opensubtitles && self.stats_panels["opensubtitles"]) {
            var os = json.opensubtitles;
            self.stats_panels["opensubtitles"].set("html", '<div class="stats_grid">' + '<div class="stat_item"><span class="stat_value">' + os.searches_today + '</span><span class="stat_label">Searches Today</span></div>' + '<div class="stat_item"><span class="stat_value">' + os.hash_hits_today + '</span><span class="stat_label">Hash Hits</span></div>' + '<div class="stat_item"><span class="stat_value">' + (os.key_type || "Unknown") + '</span><span class="stat_label">Key Type</span></div>' + "</div>");
        }
    }
});
...
var ManageSettingTab = new Class({
    ...
    createScanToolbar: function(content) {
        var self = this;
        var group = self.settings.createGroup({ label: "Library Scan", name: "library_scan" }).inject(content, "top");
        ...
        }).adopt(new Element("span.scan_btn_icon.icon-search"), new Element("span.scan_btn_label", { text: "Quick Scan" }), new Element("span.scan_btn_desc", { text: "Check for recently added or changed movies" })).inject(buttons_row);
        self.full_btn = new Element("a.scan_btn.full_refresh", {
            events: { click: self.refresh.bind(self, true) }
        }).adopt(new Element("span.scan_btn_icon.icon-refresh"), new Element("span.scan_btn_label", { text: "Full Library Refresh" }), new Element("span.scan_btn_desc", { text: "Rescan all library folders from scratch" })).inject(buttons_row);
    ...
    refresh: function(full) {
        ...
        if (!self.update_in_progress) {
            Api.request("manage.update", { data: { full: +full } });
            if (self.results_container) { self.results_container.setStyle("display", "none"); self.results_container.empty(); }
            self.startProgressInterval();
        }
    },
    setButtonsDisabled: function(disabled) {
        var self = this;
        if (self.quick_btn) {
            if (disabled) { self.quick_btn.addClass("disabled"); self.full_btn.addClass("disabled"); }
            else { self.quick_btn.removeClass("disabled"); self.full_btn.removeClass("disabled"); }
        }
    },
    startProgressInterval: function() {
        var self = this;
        if (self.progress_interval) clearRequestInterval(self.progress_interval);
            ...
            self.progress_request = Api.request("manage.progress", {
                onComplete: function(json) {
                    if (!json || !json.progress) {
                        ...
                        if (self.progress_container) { self.progress_container.setStyle("display", "none"); self.progress_container.empty(); }
                        if (json && json.results && !json.results.scanning) { self.showResults(json.results); }
                    } else {
                        ...
                        if (json.results && json.results.started) {
                            ...
                            if (elapsed_secs >= 3600) { elapsed_str = Math.floor(elapsed_secs / 3600) + "h " + Math.floor((elapsed_secs % 3600) / 60) + "m"; }
                            else if (elapsed_secs >= 60) { elapsed_str = Math.floor(elapsed_secs / 60) + "m " + (elapsed_secs % 60) + "s"; }
                            else { elapsed_str = elapsed_secs + "s"; }
                        }
                        var status_line = new Element("div.scan_status").adopt(new Element("span.scan_status_icon.icon-refresh.spinning"), new Element("span", { text: "Scanning library..." })).inject(self.progress_container);
                        if (json.results && json.results.scanning) {
                            var count_parts = [];
                            if (json.results.movies_found > 0 || json.results.movies_added > 0) { count_parts.push(json.results.movies_found + " found"); count_parts.push(json.results.movies_added + " added"); }
                            if (elapsed_str) { count_parts.push(elapsed_str + " elapsed"); }
                            if (count_parts.length > 0) { new Element("span.scan_live_count", { text: " (" + count_parts.join(", ") + ")" }).inject(status_line); }
                        }
                        ...
                        sorted_table.each(function(folder) {
                            ...
                            if (folder_progress.eta > 0) { folder_text += ", " + new Date().increment("second", folder_progress.eta).timeDiffInWords().replace("from now", "to go"); }
                            else if (folder_progress.started && done === 0) {
                                var folder_elapsed = Math.round(Date.now() / 1000 - folder_progress.started);
                                if (folder_elapsed >= 60) { folder_text += ", scanning files for " + Math.floor(folder_elapsed / 60) + "m " + (folder_elapsed % 60) + "s"; }
                                else if (folder_elapsed > 3) { folder_text += ", scanning files for " + folder_elapsed + "s"; }
                            }
                            var pct_text;
                            if (folder_progress.total && done > 0) { pct_text = Math.round(done / folder_progress.total * 100) + "%"; }
                            else if (folder_progress.total) { pct_text = "scanning\u2026"; }
                            else { pct_text = "waiting\u2026"; }
                            new Element("div.scan_folder_row").adopt(new Element("span.folder", { text: folder_text }), new Element("span.percentage", { text: pct_text })).inject(self.progress_container);
                        });
                    }
                }
            });
    ...
    showResults: function(results) {
        ...
        var time_str;
        if (elapsed < 60) { time_str = Math.round(elapsed) + "s"; }
        else { time_str = Math.round(elapsed / 60) + "m " + Math.round(elapsed % 60) + "s"; }
        var type_label = results.scan_type === "full" ? "Full refresh" : "Quick scan";
        new Element("div.scan_results_inner").adopt(new Element("span.scan_results_icon.icon-ok"), new Element("span.scan_results_text", {
            text: type_label + " complete: " + results.movies_found + " movies found, " + results.movies_added + " added to library. " + results.folders_scanned + " folder" + (results.folders_scanned !== 1 ? "s" : "") + " scanned in " + time_str + "."
        }), new Element("a.scan_results_dismiss", {
            text: "dismiss",
            events: { click: function() { self.results_container.setStyle("display", "none"); } }
        })).inject(self.results_container);
    },
    parseProgress: function(progress_object) {
        var folder, temp_array = [];
        for (folder in progress_object) { if (progress_object.hasOwnProperty(folder)) { temp_array.push(folder); } }
...
var AuditSettingTab = new Class({
	...
	renderItem: function(item){
		...
		if(!item.fixed){
			...
			// Run Identification button — always available on every card
