var AuditSettingTab = new Class({
	...
	renderItem: function(item){
		...
		// Multi-CD info
		if(item.multi_cd && item.cd_files){
			var cd_section = new Element('div.audit_detail_section').inject(detail);
			new Element('div.audit_detail_subtitle', { 'text': item.cd_count + '-CD Set:' }).inject(cd_section);
			item.cd_files.each(function(cd){
				new Element('div.audit_detail_kv', {
					'text': 'cd' + cd.cd_number + ': ' + cd.file
				}).inject(cd_section);
			});
		}
		...
		if(item.variant_files && item.variant_files.length > 1){
			var vf_section = new Element('div.audit_detail_section').inject(detail);
			new Element('div.audit_detail_subtitle', {
				'text': 'Folder Contents (' + item.variant_count + ' files):'
			}).inject(vf_section);
			item.variant_files.each(function(f){
				var is_current = (f === item.file);
				var el = new Element('div.audit_detail_kv', {
					'text': (is_current ? '\u25B6 ' : '  ') + f
				});
				if(is_current) el.setStyle('font-weight', 'bold');
				el.inject(vf_section);
			});
		}
		...
		if(!item.fixed){
				...
				new Element('a.audit_action_btn.primary', {
					...
					'events': { 'click': function(e){
						...
						if(action === 'verify_audio'){
							self.runVerify(item.item_id, e.target, card);
						} else {
			...
			var del_action = (item.recommended_action && item.recommended_action.indexOf('delete') === 0)
				? item.recommended_action : 'delete_wrong';
			new Element('a.audit_action_btn.audit_delete_btn', {
				'text': 'Delete',
				'events': { 'click': function(e){
					e.stop();
					self.showFixPreview(item.item_id, del_action);
				}}
			}).inject(actions_row);
	...
	renderBatchBar: function(){
		...
		batch_actions.each(function(ba){
			new Element('a.audit_batch_btn', {
				...
				'events': { 'click': function(e){
					e.stop();
					if(ba.action === 'verify_audio'){
						self.startBatchVerify();
					} else {
	...
	// -----------------------------------------------------------------------
	// Whisper Batch Verification
	// -----------------------------------------------------------------------

	startBatchVerify: function(){
		var self = this;

		if(!confirm('Run whisper language detection on all unknown audio items? This may take ~5 seconds per file.')){
			return;
		}

		Api.request('audit.verify.batch', {
			'onComplete': function(json){
				if(json && json.success !== false){
					self.startVerifyProgressPolling();
				} else {
					alert('Batch verification failed: ' + (json ? json.error || 'Unknown error' : 'No response'));
				}
			}
		});
	},

	startVerifyProgressPolling: function(){
		var self = this;

		if(self.verify_progress_interval)
			clearRequestInterval(self.verify_progress_interval);

		self.batch_bar.empty();
		new Element('div.audit_batch_progress').adopt(
			new Element('span.icon-refresh.spinning'),
			new Element('span.audit_batch_progress_text', { 'text': ' Whisper verification in progress...' })
		).inject(self.batch_bar);

		self.verify_progress_interval = requestInterval(function(){
			Api.request('audit.verify.progress', {
				'onComplete': function(json){
					if(!json || !json.verify_progress || !json.verify_progress.active){
						clearRequestInterval(self.verify_progress_interval);
						self.verify_progress_interval = null;
						self.loadStats();
						return;
					}

					var vp = json.verify_progress;
					var text = ' Verifying: ' + vp.completed + ' / ' + vp.total;
					if(vp.failed > 0) text += ' (' + vp.failed + ' failed)';
					if(vp.current_item) text += ' \u2014 ' + vp.current_item;
	...
	runVerify: function(item_id, btn, card){
		var self = this;

		var orig_text = btn.get ? btn.get('text') : btn.textContent;
		if(btn.set) btn.set('text', 'Verifying...');
		else btn.textContent = 'Verifying...';
		if(btn.addClass) btn.addClass('audit_full_running');
		if(btn.removeEvents) btn.removeEvents('click');

		Api.request('audit.verify', {
			'data': { 'item_id': item_id },
			'onComplete': function(json){
				if(btn.removeClass) btn.removeClass('audit_full_running');
				...
				if(json && json.success){
					...
					if(card){
						...
						if(flag_rows) flag_rows.each(function(row){
							var check_el = row.getElement('.audit_flag_check');
							if(check_el){
								...
								if(check === 'unknown_audio' || check === 'foreign_audio'){
									...
									if(json.recommended_action === 'set_audio_language')
										check_el.set('text', 'audio_mislabeled');
									else if(json.recommended_action === 'delete_foreign')
										check_el.set('text', 'foreign_audio');
									else if(json.recommended_action !== 'verify_audio')
										check_el.set('text', 'cleared');
								}
							}
						});

						// Update header flag pills
						if(json.recommended_action === 'set_audio_language'){
							card.getElements('.audit_flag_icon').each(function(icon){
								if(icon.get('title') === 'unknown_audio'){
									icon.set('text', self.flagIcon('audio_mislabeled'));
									icon.set('title', 'audio_mislabeled');
								}
							});
						}

						// Update the header action badge
						var badge = card.getElement('.audit_action_badge');
						if(badge && json.recommended_action){
							badge.set('text', self.actionLabel(json.recommended_action));
						}

						// Replace verify button with fix button when action changed
						if(json.recommended_action === 'set_audio_language'){
							var new_action = json.recommended_action;
							var fix_btn = new Element('a.audit_action_btn.primary', {
								'text': self.actionLabel(new_action),
								'events': { 'click': function(e){
									e.stop();
									self.showFixPreview(item_id, new_action);
								}}
							});
							fix_btn.inject(btn, 'before');
							btn.destroy();
						}
					}

					// Refresh stats counts without rebuilding item list
					self.refreshStatsOnly();
				} else {
					if(btn.set) btn.set('text', orig_text);
					else btn.textContent = orig_text;
					if(btn.addEvent) btn.addEvent('click', function(e){
						e.stop();
						self.runVerify(item_id, btn, card);
					});
					alert('Verification failed: ' + (json ? json.error || 'Unknown error' : 'No response'));
				}
			}
		});
	},
