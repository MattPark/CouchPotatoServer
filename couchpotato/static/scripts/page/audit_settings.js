var AuditSettingTab = new Class({
	...
	renderItem: function(item){
		...
		// Severity badge
		var max_severity = 'MEDIUM';
		if(item.flags){
			item.flags.each(function(f){
				if(f.severity === 'HIGH') max_severity = 'HIGH';
			});
		}
		...
		if(!item.fixed){
			...
			// Run Tier 2 button — always available on every card
		...
		Api.request('audit.identify', {
			...
			'onComplete': function(json){
				...
				if(json && json.success){
					// Refresh the full results list so the card rebuilds
					// with updated identification and recommended_action
					self.loadStats();
				} else {
