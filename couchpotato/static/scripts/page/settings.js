...
Option.Imdb_watchlist_urls = new Class({
	...
	create: function(){
		var self = this;
		self.el.addClass('imdb_watchlist_urls');
		...
		self.rows_el = new Element('div.imdb-watchlist-rows');
		self.add_btn = new Element('a.button.imdb-watchlist-add-btn', {
			'text': '+ Add List',
			'events': {
				'click': function(e){
					e.preventDefault();
					self._createRow('', true, '', 0);
					self._saveAll();
				}
			}
		});
		...
		self.el.adopt(
			self.input,
			self.rows_el,
			new Element('div.imdb-watchlist-footer').adopt(self.add_btn)
		);
	},
	...
	afterInject: function(){
		...
		if(entries && entries.length){
			entries.each(function(entry){
				self._createRow(entry.url || '', entry.enabled !== false, entry.title || '', entry.count || 0);
			});
		}
	},

	_createRow: function(url, enabled, title, count){
		var self = this;
		var row = new Element('div.imdb-watchlist-row');

		// Column 1: URL input (wide)
		var url_input = new Element('input.imdb-watchlist-url-input', {
			'type': 'text',
			'value': url,
		...
		// Column 2: Test button
		var test_wrapper = new Element('div.imdb-watchlist-test-wrapper');
		var test_result = new Element('span.imdb-watchlist-test-result');
		var test_btn = new Element('a.button.imdb-watchlist-test-btn', {
			'text': 'Test',
			'events': {
				'click': function(e){
					e.preventDefault();
					self._testUrl(url_input.get('value'), this, test_result, title_el, row);
				}
			}
		});
		test_wrapper.adopt(test_btn, test_result);

		// Column 3: List title (read-only, auto-populated)
		var title_el = new Element('span.imdb-watchlist-title', {
			'text': title ? title + (count ? ' (' + count + ')' : '') : ''
		});

		// Column 4: Delete button (trash can)
		var delete_btn = new Element('a.icon-delete.imdb-watchlist-delete', {
			'title': 'Remove',
			'events': {
				'click': function(e){
					e.preventDefault();
		...
		// Column 5: Enable/disable toggle
		var toggle_wrapper = new Element('label.switch.imdb-watchlist-toggle');
		var toggle_input = new Element('input', {
			'type': 'checkbox',
			'checked': enabled,
			'events': {
				'change': function(){
					row[this.checked ? 'removeClass' : 'addClass']('imdb-watchlist-disabled');
					self._saveAll();
				}
			}
		});
		...
		row.adopt(url_input, test_wrapper, title_el, delete_btn, toggle_wrapper);

		if(!enabled) row.addClass('imdb-watchlist-disabled');

		// Store data references on the row element
		row.store('url_input', url_input);
		row.store('toggle_input', toggle_input);
		row.store('title_el', title_el);
		row.store('title', title || '');
		row.store('count', count || 0);
	...
	_testUrl: function(url, btn, result_el, title_el, row){
		if(!url){
			result_el.set('html', '<span class="imdb-fail" title="No URL entered">\u2716</span>');
			return;
		}
		btn.set('text', '...');
		result_el.set('html', '');
		...
		var self = this;
		Api.request('imdb.test_url', {
			'data': {'url': url},
			'onComplete': function(json){
				btn.set('text', 'Test');
				if(json.success){
					var title_text = json.title || 'List';
					var count = json.count || 0;
					var display = title_text + (count ? ' (' + count + ')' : '');
					result_el.set('html', '<span class="imdb-ok" title="' + display.replace(/"/g, '&quot;') + '">\u2714</span>');
					title_el.set('text', display);
					row.store('title', title_text);
					row.store('count', count);
					self._saveAll();
				} else {
					var msg = (json.message || 'Failed').replace(/"/g, '&quot;');
					result_el.set('html', '<span class="imdb-fail" title="' + msg + '">\u2716</span>');
				}
			}
		});
	},
	...
	_saveAll: function(){
		...
		self._rows.each(function(row){
			...
			var title = row.retrieve('title') || '';
			var count = row.retrieve('count') || 0;
			entries.push({
...
Option.Apprise_urls = new Class({
	...
	_createRow: function(schema, url, enabled, on_snatch){
		...
		var delete_btn = new Element('a.icon-delete.apprise-delete', {
			'title': 'Remove',
			'events': {
				'click': function(e){
					e.preventDefault();
